;; Generated from http://m.modelling4all.org/m/?frozen=Wa-rFeoeik2wZbDHKyBZ7c
;; on Mon Sep 17 03:49:37 UTC 2012
...
patches-own [land-population-of-patch 
 original-population-of-patch 
 medium-distance-travel-destinations-of-patch 
 latitude-of-patch 
 longitude-of-patch 
...
to the-model 
   ...
   ask all-of-kind "World" 
       [ WORLD-MAP-02941 
         INITIALISE-02944 
         PEACETIME-ADDITIONAL-TRAFFIC-02947 
         AUSTRALIAN-RETURN-TRAFFIC-02948 
         SCHEDULE-MOVEMENTS-02949 
         SET-UP-INDEX-CASE-02953 
         SCHEDULE-ODDS-CHANGES-02955 
         ARMISTICE-CELEBRATIONS-NORTH-AMERICAN-AND-EUROPE-02957 
         ARMISTICE-CELEBRATIONS-AUSTRALIA-AND-NEW-ZEALAND-02958 
         SCHEDULE-ENCOUNTER-RATE-CHANGES-02959 
         COMPUTE-GLOBAL-INFECTED-02971 
         COMPUTE-GLOBAL-SUSCEPTIBLES-02972 
         COMPUTE-GLOBAL-RECOVERED-02973 ] 
   ask all-of-kind "Parameters" [ ] 
   ask all-of-kind "Observer" 
       [ VIEW-SUB-POPULATION-03004 
         MULTIPLE-PLOT-GENERATIONS-03010 
         PLOT-SUSCEPTIBLES-03011 
         PLOT-INFECTED-03012 
         PLOT-RECOVERED-03013 
         PLOT-DEATHS-03014 
         PLOT-DAILY-DEATHS-03016 
         PLOT-DAILY-INFECTED-03017 
         PLOT-DAILY-RECOVERED-03018 
         SUMMARIZE-OUTCOME-03019 
         PAUSE-WHEN-NO-ONE-INFECTED-03020 
         PAUSE-WHEN-1-JAN-1920-03021 ] 
end

to-report aef-movements [ return-too ] 
   let us-army-camps objects with [ is-army-camp? and is-in-continental-usa? ] 
   let western-europe-places objects with [ is-stationary? and in-western-europe? ] 
   let america-odds [ [ .77 "New York" ] [ .15 "Newport News" ] [ .025 "Montreal" ] [ .025 "Philadelphia" ] [ .02 
                                                                                                              "Boston" ] 
                      [ .01 "Quebec City" ] ] 
   let europe-odds [ [ .435 "Brest" ] [ .42 "Liverpool" ] [ .03 "Southampton" ] [ .03 "London" ] [ .025 "Glasgow" ] 
                     [ .025 "Bordeaux" ] [ .025 "Saint-Nazaire" ] [ .01 "Le Havre" ] ] 
   let june-1 151 
   ...
   forEach ( sentence monthly-to-europe-totals-1917 monthly-to-europe-totals-1918 0 ) 
           [ if first-of-the-month > 0 
                [ let daily-flow-per-camp ? * 1000 / ( days-in-month * number-of-camps ) 
                  do-at-time 
                  round first-of-the-month 
                             task [ ask us-army-camps 
                                        [ set my-daily-flow daily-flow-per-camp ] ] ] 
   ...
   if the-first-year < 1918 
      [ set day june-1 
        forEach monthly-to-europe-totals-1917 
                [ set movements sentence movements ( generate-n-movements ? 
                                                     round day 1917 america-odds europe-odds us-army-camps ) 
                  set day day + days-in-month ] ] 
   set day 0 
   forEach monthly-to-europe-totals-1918 
           [ set movements sentence movements ( generate-n-movements ? 
                                                round day 1918 america-odds europe-odds us-army-camps ) 
             set day day + days-in-month ] 
   if return-too 
      [ set day 304 
        forEach monthly-to-america-totals-1918 
                [ set movements sentence movements ( generate-n-movements ? 
                                                     round day 1918 europe-odds america-odds western-europe-places ) 
                  set day day + days-in-month ] 
        set day 0 
        forEach monthly-to-america-totals-1918 
                [ set movements sentence movements ( generate-n-movements ? 
                                                     round day 1919 europe-odds america-odds western-europe-places ) 
                  set day day + days-in-month ] ] 
   report movements 
end
...
to-report generate-n-movements [ embarking day year source-odds destination-odds originating-places ] 
   ...
   let ship-count 
   round ( embarking / thousands-each-trip ) 
   let ship-frequency 30 / ship-count 
   report n-values ship-count [ generate-round-trip ( day + 
                                                      round ( ? * ship-frequency ) ) year source-odds destination-odds 
                                                            originating-places ] 
end
...
to-report generate-round-trip [ day year source-odds destination-odds originating-places ] 
   ...
   report ( list ( list day year ) ( list ( list originating-places 4 1 0 5000 "train" ) ( list source-port 10 8 200 200 
                                                                                           "train" ) ( list 
                                                                                                       destination-port 
                                                                                                       10 8 5000 1000 ) 
                                     ( list source-port 10 1 200 200 "train" ) ( list originating-places 4 0 500 0 
                                                                                 "train" ) ) 1 ) 
end
...
to-report log-as-hue [ sub-population ] 
   ...
   if sub-population < .5 or population < 1 
      [ if-else is-transport? or is-army-camp? 
                [ report green ] 
                [ report [ 0 0 0 0 ] ] ] 
   let fraction-of-population minimum 1 ( sub-population / population ) 
   let log-fraction log fraction-of-population 10 
   if log-fraction < -6 
      [ report [ 0 0 0 0 ] ] 
   let hue 
   round ( ( 255 * ( 6 + log-fraction ) / 12 ) + 127.5 ) 
   report hsb hue 255 255 
end

to-report transport-size 
   report maximum 2 minimum 6 ( 6 * total-population / 5000 ) 
end
   ...
   if is-agent? source and amount > 0 
      [ let source-total [ next-total-population ] of source 
        if-else ( source-total > 0 ) 
                [ if amount > source-total 
                     [ set amount source-total ] 
                  let fraction amount / source-total 
                  transfer-from-sub-population source self fraction * [ my-next-infected ] of source fraction * 
                  [ my-next-susceptibles ] of source fraction * [ my-next-recovered ] of source fraction * [ 
                                                                                                             my-next-others ] 
                                               of source ifelse-value dead-too [ [ my-next-dead ] of source ] 
                  [ 0 ] no-known-infected ] [ 
                                              if amount >= 1 
                                                 [ show ( list "No population to transfer from" source "x" pxcor "y" 
                                                          pycor "amount" amount "time" time ) ] ] ] 
end

to transfer-to [ 
                 destination amount dead-too no-known-infected ] 
   if is-agent? 
   destination 
      [ ask 
        destination 
            [ transfer-from myself amount dead-too no-known-infected ] ] 
end

to transfer-from-sub-population [ source 
                                  destination infected-transferred susceptibles-transferred recovered-transferred 
                                              others-transferred dead-transferred no-known-infected ] 
   if no-known-infected and infected-transferred > 0 
      [ let known-infected infected-transferred * the-fraction-infected-known-to-be-infected 
        ...
        let susceptibles-fraction recovered-transferred / ( susceptibles-transferred + recovered-transferred ) 
        set susceptibles-transferred susceptibles-transferred + known-infected * susceptibles-fraction 
        set recovered-transferred recovered-transferred + known-infected * ( 1.0 - susceptibles-fraction ) ] 
   ask source 
       [ set my-next-infected my-next-infected - infected-transferred 
         set my-next-susceptibles my-next-susceptibles - susceptibles-transferred 
         set my-next-recovered my-next-recovered - recovered-transferred 
         set my-next-others my-next-others - others-transferred 
         set my-next-dead my-next-dead - dead-transferred 
...
to-report is-quarantined? [ population ] 
   report ( is-historical? or is-war-ends-1920? ) and [ is-in-australia? ] of population and is-fall-1918? 
end
...
to-report is-fall-1918? 
   ...
   if time < fall-start 
      [ report false ] 
   if time > fall-start + 120 
      [ report false ] 
   report true 
end

to-report total-population 
   report my-susceptibles + my-infected + my-recovered + my-others 
end

to-report next-total-population 
   report my-next-susceptibles + my-next-infected + my-next-recovered + my-next-others 
end

to-report total-land-population-here 
   if-else is-agent? 
   land-population-of-patch 
           [ report [ total-population ] of 
             land-population-of-patch ] [ 
                                          report 0 ] 
end
...
to-report closest-land-population [ x y number-moving ] 
   ...
   if is-agent? 
   land-population-of-patch and [ total-population > number-moving ] of 
   land-population-of-patch 
      [ report 
        land-population-of-patch ] 
   let populated-neighbors [ neighbors4 with [ is-agent? 
                                               land-population-of-patch and [ total-population > number-moving ] of 
                                               land-population-of-patch ] ] of patch x y 
   if any? populated-neighbors 
      [ report [ 
                 land-population-of-patch ] of min-one-of populated-neighbors [ distancexy x y ] ] 
   report nobody 
end

to-report closest-army-camp [ x y ] 
   report min-one-of ( objects with [ is-army-camp? ] ) [ distancexy x y ] 
end
...
to travel 
   ...
   let medium-distance-travelers travelers * the-medium-distance-travel-fraction 
   set travelers travelers - medium-distance-travelers 
   let 
   source-population self 
   let destinations neighbors4 with [ original-population-of-patch > 1 ] 
   if any? destinations and ( my-infected > .5 or any? destinations with [ [ my-infected ] of 
                                                                           land-population-of-patch > .5 ] ) 
      [ let destinations-total sum [ original-population-of-patch ] of destinations 
        ask destinations 
            [ ask 
              land-population-of-patch 
                  [ let fraction-of-destinations original-population-of-patch / destinations-total 
                    let travelers-to-destination travelers * fraction-of-destinations 
                    if travelers-to-destination >= 1 
                       [ transfer-from 
                         source-population travelers-to-destination false true 
                         do-after 1 
                                  task [ transfer-to 
                                         source-population travelers-to-destination false true ] ] ] ] ] 
   if any? medium-distance-travel-destinations-of-patch and ( my-infected > .5 or any? 
                                                              medium-distance-travel-destinations-of-patch with 
                                                              [ [ my-infected ] of 
                                                                land-population-of-patch > .5 ] ) 
      [ let travelers-to-each-distant-destination medium-distance-travelers / count 
            medium-distance-travel-destinations-of-patch 
        ask medium-distance-travel-destinations-of-patch 
            [ ask 
              land-population-of-patch 
                  [ transfer-from 
                    source-population travelers-to-each-distant-destination false true 
                    do-after 7 
                             task [ transfer-to 
                                    source-population travelers-to-each-distant-destination false true ] ] ] ] 
end
...
to-report population-movements 
   ...
   if is-historical? or is-war-ends-1920? 
      [ set movements sentence aef-movements is-historical? movements ] 
   report map [ ( list ifelse-value is-number? first ? [ first ? ] [ compute-time first first ? second first ? ] 
                  ( map pre-process-step second ? ) ifelse-value ( length ? > 2 ) [ item 2 ? ] [ 1000000 ] ) ] movements 
end

to-report pre-process-step [ step ] 
   let place place-from-step item 0 step 
   let duration item 1 step 
   let next-delay ifelse-value ( length step > 2 ) [ item 2 step ] [ 0 ] 
   let disembark ifelse-value ( length step > 3 ) [ item 3 step ] [ 0 ] 
   let embark ifelse-value ( length step > 4 ) [ item 4 step ] [ 0 ] 
   let shape-name ifelse-value ( length step > 5 ) [ item 5 step ] [ "boat top" ] 
   if shape-name = "train" 
      [ set shape-name "train freight engine" ] 
   if shape-name = "boat" 
      [ set shape-name "boat top" ] 
   report ( list place duration next-delay disembark embark shape-name ) 
end

to to-camp [ nationality number-entering ] 
   let land-populations populations-of-country nationality 
   let country-total sum [ total-population ] of land-populations 
   let fraction-entering number-entering / country-total 
   let 
   destination self 
   ask land-populations 
       [ let entering-here total-population * fraction-entering 
         if entering-here > .5 
            [ transfer-to 
              destination entering-here false true ] ] 
end

to from-camp [ nationality number-leaving ] 
   let land-populations populations-of-country nationality 
   let country-total sum [ total-population ] of land-populations 
   let fraction-leaving number-leaving / country-total 
   let source self 
   ask land-populations 
       [ let leaving-from-here total-population * fraction-leaving 
   ...
   if-else remaining-path = [ ] 
           [ if-else repeat-count > 1 
                     [ let reversed-path reverse original-path 
                       follow-path but-first reversed-path reversed-path repeat-count - 1 ] 
                     [ set dead true ] ] [ 
                                           ...
                                           if is-agentset? next-place 
                                              [ let chosen-place max-one-of next-place [ total-population ] 
                                                set original-path replace-places-with-place chosen-place next-place 
                                                    original-path 
                                                set next-place chosen-place ] 
                                           let distance-to-travel ifelse-value is-list? next-place [ [ distancexy item 0 
                                                                                                       next-place item 1 
                                                                                                       next-place ] of 
                                                                                                     patch-here ] 
                                           ...
                                           if-else is-list? next-place 
                                                   ...
                                                   [ face next-place ] 
                                           do-after 1 
                                                    task [ path-step-movement next-place distance-to-travel delta 
                                                                              disembarking embarking next-delay 
                                                                              but-first remaining-path original-path 
                                                                              repeat-count ] ] 
end

to path-step-movement [ next-place distance-to-travel delta disembarking embarking next-delay remaining-path 
                        original-path repeat-count ] 
   if-else distance-to-travel <= delta 
           [ if-else is-agent? next-place 
             ...
             if-else remaining-path = [ ] and repeat-count <= 1 
                     [ set dead true ] 
                     [ do-after next-delay 
                                task [ follow-path remaining-path original-path repeat-count ] ] 
             if disembarking > 0 or embarking > 0 
                [ if-else is-agent? 
                  land-population-of-patch 
                          [ if not is-quarantined? 
                            land-population-of-patch 
                               [ transfer-to 
                                 land-population-of-patch minimum next-total-population disembarking true false 
                                 transfer-from 
                                 land-population-of-patch embarking true false ] ] [ 
                                                                                     show ( list 
                                                                                            "no land counter part at " 
                                                                                            xcor ycor time disembarking 
                                                                                            embarking remaining-path 
                                                                                            original-path ) ] ] ] 
                 [ 
                   forward delta 
                   do-after 1 
                            task [ path-step-movement next-place distance-to-travel - delta delta disembarking embarking 
                                                      next-delay remaining-path original-path repeat-count ] ] 
end

to-report replace-places-with-place [ chosen-place places path ] 
   report map [ ifelse-value ( first ? = places ) [ fput chosen-place butFirst ? ] [ ? ] ] path 
end

to update-initial-population [ population additional-susceptibles additional-others additional-population ] 
   ask population 
       [ set my-susceptibles my-susceptibles + additional-susceptibles 
         set my-others my-others + additional-others 
         set original-population-of-patch original-population-of-patch + additional-population ] 
end

to adjust-infection-and-mortality-odds 
   ask objects 
       [ set my-infection-odds the-average-infection-odds ] 
   ask objects 
       [ set my-mortality-odds the-average-mortality-odds ] 
...
to-report place-from-step [ place ] 
   if is-agent? place or is-agentset? place 
      [ report place ] 
   if is-list? place 
      [ report ( list longitude-to-x item 1 place latitude-to-y item 0 place ) ] 
   if is-string? place 
      [ forEach the-places 
                [ if first ? = place 
                     [ report ( list longitude-to-x item 2 ? latitude-to-y item 1 ? ) ] ] ] 
           ...
           [ let next-y first ? 
             ...
             if-else ( latitude > next-latitude ) 
                     ...
                     [ if lower-latitude >= -90 
                          [ let gap-latitude next-latitude - lower-latitude 
                            let gap-y next-y - lower-y 
                            let gap-fraction ( latitude - lower-latitude ) / gap-latitude 
                            report lower-y + gap-fraction * gap-y ] ] ] 
   report max-pycor 
end
           ...
           [ let next-y first ? 
             ...
             if-else y > next-y 
                     ...
                     [ if lower-y >= 0 
                          [ let gap-latitude next-latitude - lower-latitude 
                            let gap-y next-y - lower-y 
                            let gap-fraction ( y - lower-y ) / gap-y 
                            report lower-latitude + gap-fraction * gap-latitude ] ] ] 
   report -55.5 
end

to-report location-description [ x y ] 
   let latitude-1 
   round y-to-latitude y 
   let latitude-2 
   round y-to-latitude ( y + 1 ) 
   let longitude-1 
   round x-to-longitude x 
   let longitude-2 
   round x-to-longitude ( x + 1 ) 
   report ( word abs latitude-1 " " north-south latitude-1 " to " abs latitude-2 " " north-south latitude-2 "; " abs 
            longitude-1 " " east-west longitude-1 " to " abs longitude-2 " " east-west longitude-2 ) 
end
...
to-report is-transport? 
   report shape = "boat top" or shape = "train freight engine" 
end
...
to-report is-in-australia? 
   report latitude-of-patch <= -9.45 and latitude-of-patch >= -43.77 and longitude-of-patch >= 111.34 and 
          longitude-of-patch <= 155.73 
end

to-report is-in-us-or-canada? 
   report latitude-of-patch <= 83 and latitude-of-patch >= 27.37 and longitude-of-patch >= -173 and longitude-of-patch 
          <= -52 
end

to-report is-in-continental-usa? 
   report latitude-of-patch <= 49 and latitude-of-patch >= 27.37 and longitude-of-patch >= -173 and longitude-of-patch 
          <= -52 
end

to-report is-in-latin-america? 
   report latitude-of-patch <= 27.36 and latitude-of-patch >= -56 and longitude-of-patch >= -122 and longitude-of-patch 
          <= -33 
end

to-report is-in-sub-sahara-africa? 
   report latitude-of-patch <= 26 and latitude-of-patch >= -35 and longitude-of-patch >= -18 and longitude-of-patch <= 
          52 
end

to-report is-in-north-africa? 
   report latitude-of-patch <= 37.46 and latitude-of-patch >= 25.99 and longitude-of-patch >= -27 and longitude-of-patch 
          <= 52 
end
...
to-report is-in-europe? 
   report latitude-of-patch <= 71 and latitude-of-patch >= 37.45 and longitude-of-patch >= -20 and longitude-of-patch <= 
          32 
end
...
to-report is-in-britain? 
   report latitude-of-patch <= 68.8 and latitude-of-patch >= 44 and longitude-of-patch >= -6.7 and longitude-of-patch <= 
          -0.5 
end

to-report is-in-northern-france? 
   report latitude-of-patch <= 51.1 and latitude-of-patch >= 46 and longitude-of-patch >= 0 and longitude-of-patch <= 5 
end

to-report is-in-asia? 
   report ( latitude-of-patch <= 77 and latitude-of-patch >= -10 and longitude-of-patch >= 32 or longitude-of-patch <= 
            -173 ) and not is-in-africa? 
end
           ...
           [ let area patch floor mouse-xcor floor mouse-ycor 
             ...
             if-else any? populations 
                     [ let total sum [ total-population ] of populations 
                       report ( word "total=" to-word 
                                round total 7 "; infected = " percent 
                                round sum [ my-infected ] of populations total 5 "; susceptibles = " percent 
                                round sum [ my-susceptibles ] of populations total 1 "; recovered = " percent 
                                round sum [ my-recovered ] of populations total 1 "; dead = " percent 
                                round sum [ my-dead ] of populations total 5 " at " location-description floor 
                                      mouse-xcor floor mouse-ycor ) ] 
                     [ report "Zero population" ] ] [ 
                                                      ...
                                                      if-else times-scheduled = 0 
                                                              ...
                                                              [ if-else time <= 1.0E-6 
                                                                        ...
                                                                        [ report 
                                                                                 "Move mouse over an area to display population statistics. Move off map to restore display." ] ] ] 
end
...
to-report date [ day year ] 
   if day > 334 
      [ report date-string ( day - 334 ) "Dec" year ] 
   if day > 304 
      [ report date-string ( day - 304 ) "Nov" year ] 
   if day > 273 
      [ report date-string ( day - 273 ) "Oct" year ] 
   if day > 243 
      [ report date-string ( day - 243 ) "Sep" year ] 
   if day > 212 
      [ report date-string ( day - 212 ) "Aug" year ] 
   if day > 181 
      [ report date-string ( day - 181 ) "Jul" year ] 
   if day > 151 
      [ report date-string ( day - 151 ) "Jun" year ] 
   if day > 120 
      [ report date-string ( day - 120 ) "May" year ] 
   if day > 90 
      [ report date-string ( day - 90 ) "Apr" year ] 
   if day > 59 
      [ report date-string ( day - 59 ) "Mar" year ] 
   if day > 31 
      [ report date-string ( day - 31 ) "Feb" year ] 
   report date-string day "Jan" year 
end

to-report date-string [ day month year ] 
   report ( word ifelse-value ( day < 10 ) [ ( word " " day ) ] [ day ] " " month " " year ) 
end
...
to-report darken-by-generation [ c ] 
   ...
   report approximate-hsb item 0 hue-staturation-brightness item 1 hue-staturation-brightness ( maximum 0 ( ( item 2 
...
to-report lighten-by-generation [ c ] 
   let hue-staturation-brightness extract-hsb c 
   report approximate-hsb item 0 hue-staturation-brightness item 1 hue-staturation-brightness ( minimum 255 ( 
                                                                                                              ( item 2 
                                                                                                                hue-staturation-brightness ) 
                                                                                                              + 
                                                                                                              ( 
...
to reset 
   set the-first-year 0 setup 
end

to-report reset-or-resume 
   if the-previous-index-case != index-case and not uninitialised? index-case 
      [ clear-patches 
        clear-drawing 
        clear-turtles 
        clear-plot setup ] 
   if the-previous-history != history and not uninitialised? history [ setup ] 
   if is-no-war? and index-case = "Etaples" and ( the-previous-history != history or the-previous-index-case != 
                                                  index-case ) 
      [ output-print 
                     "It is unlikely that Etaples would have been the origin of the influenza with no war. There would not have been a military base there." ] 
...
to TRANSMIT-02931 
   do-every ( 1 ) 
            task [ if ( my-infected > .5 ) 
                      [ let fraction-not-infected 1 - ( my-infected / total-population ) 
                        let odds-of-encountering-infected 1 - fraction-not-infected ^ my-encounter-rate 
                        let newly-infected my-susceptibles * odds-of-encountering-infected * my-infection-odds 
                        set my-next-infected-set true 
                        set my-next-infected my-next-infected + newly-infected 
                        set my-next-susceptibles-set true 
                        set my-next-susceptibles my-next-susceptibles - newly-infected 
                        set the-newly-infected the-newly-infected + newly-infected 
                        set the-total-infections the-total-infections + newly-infected ] ] 
end
...
to MORTALITY-02933 
   do-every ( 1 ) 
            task [ let deceased my-infected * my-mortality-odds / the-average-duration 
                   set my-next-infected-set true 
...
to SECOND-WAVE-02934 
   do-after ( random-integer-between 1 21 ) 
            task [ do-after ( compute-time 230 1918 ) 
                            task [ let fraction-becomes-susceptible random-number-between .06 .65 
                                   let newly-susceptible my-recovered * fraction-becomes-susceptible 
                                   set my-next-susceptibles-set true 
                                   set my-next-susceptibles my-next-susceptibles + newly-susceptible 
                                   set my-next-recovered-set true 
                                   set my-next-recovered my-next-recovered - newly-susceptible ] ] 
end

to THIRD-WAVE-02935 
   do-after ( random-integer-between 1 21 ) 
            task [ do-after ( compute-time 15 1919 ) 
                            task [ let fraction-becomes-susceptible random-number-between .91 .97 
                                   let newly-susceptible my-recovered * fraction-becomes-susceptible 
                                   set my-next-susceptibles-set true 
                                   set my-next-susceptibles my-next-susceptibles + newly-susceptible 
                                   set my-next-recovered-set true 
...
to ENCOUNTER-RATE-02936 
   set my-encounter-rate 3 * the-encounter-rate 
end

to TRAVEL-RATE-02937 
   set my-travel-rate .01 
end

to POSSIBLE-SHAPES-02938 
   set shape "boat top" 
...
to UPDATE-COLOR-02940 
   do-every ( 1 ) 
            task [ set color compute-color ] 
end

to WORLD-MAP-02941 run-in-observer-context task [ import-drawing "world.png" ] 
end
...
to INITIALISE-02944 
   ...
   forEach the-initial-state 
           [ let longitude item 0 ? 
             ...
             let patch1-land-population [ one-of objects-here with [ not dead and kind = "Population" and is-stationary? ] ] 
                 of patch1 
             if-else is-agent? patch1-land-population 
                     [ update-initial-population patch1-land-population patch1-susceptibles patch1-others 
                                                 patch1-population ] 
                     [ set just-created-agents nobody 
                       ...
                       ask just-created-agents 
                           [ set xcor xcor 
                             set ycor ycor 
                             initialise-object initialise-previous-state 
                             TRANSMIT-02931 
                             RECOVER-02932 
                             MORTALITY-02933 
                             SECOND-WAVE-02934 
                             THIRD-WAVE-02935 
                             ENCOUNTER-RATE-02936 
                             TRAVEL-RATE-02937 
                             POSSIBLE-SHAPES-02938 
                             SIZE-02939 
                             UPDATE-COLOR-02940 
                       ...
                       ask patch1 
                           [ set longitude-of-patch longitude 
                             ...
                             set original-population-of-patch patch1-population ] ] 
             let patch2-land-population [ one-of objects-here with [ not dead and kind = "Population" and is-stationary? ] ] 
                 of patch2 
             if-else is-agent? patch2-land-population 
                     [ update-initial-population patch2-land-population patch2-susceptibles patch2-others 
                                                 patch2-population ] 
                     [ set just-created-agents nobody 
                       ask patch-here 
                           [ sprout-objects 1 [ 
                       ...
                       ask just-created-agents 
                           [ set xcor xcor 
                             ...
                             do-every 1 
                                      task [ travel ] ] 
                       ask patch2 
                           [ set longitude-of-patch longitude + longitude-of-a-patch 
                             set latitude-of-patch latitude 
                             set original-population-of-patch patch2-population ] ] ] 
   ask objects with [ kind = "Population" and total-population < 1 ] 
       [ set dead true ] 
   set the-populations-of-usa objects with [ is-stationary? and is-in-continental-usa? ] 
   set the-populations-of-britain-northern-france objects with [ is-stationary? and ( is-in-britain? or 
                                                                                      is-in-northern-france? ) ] 
   set the-populations-of-britain objects with [ is-stationary? and is-in-britain? ] 
   set the-populations-of-northern-france objects with [ is-stationary? and is-in-northern-france? ] 
   ask patches with [ true ] 
       [ set medium-distance-travel-destinations-of-patch ( max-n-of the-medium-distance-travel-destinations-count other 
                                                            patches in-radius the-maximum-medium-distance-travel 
                                                            [ original-population-of-patch ] ) with [ 
                                                                                                      original-population-of-patch 
                                                                                                      > 0 ] 
         set 
         land-population-of-patch one-of objects-here with [ not dead and kind = "Population" and is-stationary? ] ] 
   if ( is-historical? or is-war-ends-1920? ) and not uninitialised? the-army-camps 
                            ...
                            ask just-created-agents 
                                [ set xcor xcor 
                                  ...
                                  when task [ the-global-susceptibles > 0 ] 
                                       task [ to-camp nationality-of-troops population ] 
                                  if nationality-of-troops != "USA" 
                                     [ if nationality-of-troops = "Britain and Northern France" 
                                       ...
                                       do-after 1 
                                                task [ do-every 1 
                                                                task [ from-camp nationality-of-troops my-daily-flow ] ] ] 
                                  do-every 1 
                                           task [ travel ] ] ] ] ] 
end

to PEACETIME-ADDITIONAL-TRAFFIC-02947 
   if ( is-no-war? ) 
      [ set the-population-movements sentence the-population-movements [ [ 1 [ [ "Philadelphia" 7 4 3000 3000 ] 
                                                                               [ "London" 7 4 3000 3000 ] ] ] 
                                                                         [ 3 [ [ "Glasgow" 7 4 3000 3000 ] [ "Boston" 7 
                                                                                                             4 3000 3000 ] ] ] 
                                                                         ...
                                                                         [ 2 [ [ "Glasgow" 10 3 3000 3000 ] [ 
                                                                               ...
                                                                               [ "Auckland" 2 3 1000 1000 ] [ 
                                                                                                              ...
                                                                                                              4 3 3000 
                                                                                                              3000 ] ] ] 
                                                                         [ 5 [ [ "Sydney" 4 3 3000 3000 ] [ "Auckland" 
                                                                                                            14 3 100 100 ] 
                                                                               [ "Panama Canal" 10 3 1000 1000 ] 
                                                                               [ "Southampton" 2 3 3000 3000 ] ] ] 
                                                                         [ 8 [ [ "London" 10 3 3000 3000 ] [ 
                                                                                                             "Panama Canal" 
                                                                                                             14 3 100 
                                                                                                             100 ] 
                                                                               [ "Auckland" 2 3 1000 1000 ] [ "Sydney" 4 
                                                                                                              3 3000 
                                                                                                              3000 ] ] ] 
                                                                         [ 15 [ [ "Melbourne" 4 3 3000 3000 ] 
                                                                                [ "Auckland" 14 3 100 100 ] [ 
                                                                                                              "Panama Canal" 
                                                                                                              10 3 1000 
                                                                                                              1000 ] 
                                                                                [ "Liverpool" 2 3 3000 3000 ] ] ] 
                                                                         [ 3 [ [ "London" 3 8 3000 3000 ] [ "Lisbon" 2 2 
                                                                                                            100 100 ] 
                                                                               [ [ 25 -22 ] 2 0 0 0 ] [ "Dakar" 4 2 100 
                                                                                                        100 ] 
                                                                               [ "Cape Town" 4 2 500 500 ] [ "Calcutta" 
                                                                                                             8 3 3000 
                                                                                                             3000 ] ] ] 
                                                                         [ 19 [ [ "Bristol" 3 8 3000 3000 ] [ "Lisbon" 2 
                                                                                                              2 100 100 ] 
                                                                                [ [ 25 -22 ] 2 0 0 0 ] [ "Dakar" 4 2 100 
                                                                                                         100 ] 
                                                                                [ "Cape Town" 4 2 500 500 ] [ "Bombay" 8 
                                                                                                              3 3000 
                                                                                                              3000 ] ] ] ] ] 
end

to AUSTRALIAN-RETURN-TRAFFIC-02948 
   if ( history = "Historical" ) 
      [ set the-population-movements sentence the-population-movements [ [ [ 345 1918 ] [ [ "Brest" 10 3 10000 10000 ] 
                                                                                          [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                          [ "Dakar" 4 1 100 100 ] 
                                                                                          [ "Cape Town" 4 1 100 100 ] 
                                                                                          [ [ -42 147 ] 9 0 0 0 ] 
                                                                                          [ "Sydney" 3 3 5000 5000 ] ] 4 ] 
                                                                         [ [ 355 1918 ] [ [ "Le Havre" 10 3 10000 10000 ] 
                                                                                          [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                          [ "Dakar" 4 1 100 100 ] 
                                                                                          [ "Cape Town" 4 1 100 100 ] 
                                                                                          [ "Melbourne" 12 3 5000 5000 ] ] 
                                                                           4 ] [ [ 1 1919 ] [ [ "Brest" 10 3 10000 10000 ] 
                                                                                              [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                              [ "Dakar" 4 1 100 100 ] 
                                                                                              [ "Cape Town" 4 1 100 100 ] 
                                                                                              [ [ -42 147 ] 9 0 0 0 ] 
                                                                                              [ "Sydney" 3 3 5000 5000 ] ] 
                                                                                 4 ] [ [ 10 1919 ] [ [ "Le Havre" 10 3 
                                                                                                       10000 10000 ] 
                                                                                                     [ [ 25 -22 ] 2 0 0 
                                                                                                       0 ] [ "Dakar" 4 1 
                                                                                                             100 100 ] 
                                                                                                     ...
                                                                                                     [ "Melbourne" 12 3 
                                                                                                       5000 5000 ] ] 4 ] 
                                                                         [ [ 25 1919 ] [ [ "Brest" 10 3 10000 10000 ] 
                                                                                         [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                         [ "Dakar" 4 1 100 100 ] 
                                                                                         [ "Cape Town" 4 1 100 100 ] 
                                                                                         [ [ -42 147 ] 9 0 0 0 ] 
                                                                                         [ "Sydney" 3 3 5000 5000 ] ] 4 ] 
                                                                         [ [ 40 1919 ] [ [ "Le Havre" 10 3 10000 10000 ] 
                                                                                         [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                         [ "Dakar" 4 1 100 100 ] 
                                                                                         [ "Cape Town" 4 1 100 100 ] 
                                                                                         [ "Melbourne" 12 3 5000 5000 ] ] 
                                                                           4 ] [ [ 55 1919 ] [ [ "Brest" 10 3 10000 
                                                                                                 10000 ] [ [ 25 -22 ] 2 
                                                                                                           0 0 0 ] 
                                                                                               [ "Dakar" 4 1 100 100 ] 
                                                                                               [ "Cape Town" 4 1 100 100 ] 
                                                                                 ...
                                                                                 4 ] [ [ 70 1919 ] [ [ "Le Havre" 10 3 
                                                                                                     ...
                                                                                                     [ "Melbourne" 12 3 
                                                                                                       5000 5000 ] ] 4 ] 
                                                                         [ [ 80 1919 ] [ [ "Brest" 10 3 10000 10000 ] 
                                                                                         [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                         [ "Dakar" 4 1 100 100 ] 
                                                                                         [ "Cape Town" 4 1 100 100 ] 
                                                                                         [ [ -42 147 ] 9 0 0 0 ] 
                                                                                         [ "Sydney" 3 3 5000 5000 ] ] 4 ] 
                                                                         [ [ 90 1919 ] [ [ "Le Havre" 10 3 10000 10000 ] 
                                                                                         [ [ 25 -22 ] 2 0 0 0 ] 
                                                                                         [ "Dakar" 4 1 100 100 ] 
                                                                                         [ "Cape Town" 4 1 100 100 ] 
                                                                                         [ "Melbourne" 12 3 5000 5000 ] ] 
...
to SCHEDULE-MOVEMENTS-02949 
   let just-created-agents nobody 
   do-after ( 1 ) 
            task [ forEach population-movements 
                           [ let start-time item 0 ? 
                             if is-list? start-time 
                                [ set start-time compute-time first start-time second start-time ] 
                             let path item 1 ? 
                             ...
                             let from-population ifelse-value is-list? from-place [ closest-land-population item 0 
                                                                                    from-place item 1 from-place 
                                                                                    number-moving ] [ from-place ] 
                             let shape-name item 5 initial-step 
                             if-else is-agent? from-population or is-agentset? from-population 
                                     [ if start-time >= 0 
                                          [ do-at-time start-time 
                                                       task [ if is-agentset? from-population 
                                                                 [ set from-population max-one-of from-population 
                                                                   [ total-population ] ] 
                                                              ask from-population 
                                                                  [ let total total-population 
                                                                    if-else total >= number-moving 
                                                                            [ let fraction number-moving / total 
                                                                              let susceptibles-moving fraction * 
                                                                                  my-next-susceptibles 
                                                                              set my-next-susceptibles 
                                                                                  my-next-susceptibles - 
                                                                                  susceptibles-moving 
                                                                              let infected-moving fraction * 
                                                                                  my-next-infected 
                                                                              set my-next-infected my-next-infected - 
                                                                                  infected-moving 
                                                                              let recovered-moving fraction * 
                                                                                  my-next-recovered 
                                                                              set my-next-recovered my-next-recovered - 
                                                                                  recovered-moving 
                                                                              let others-moving fraction * 
                                                                                  my-next-others 
                                                                              set my-next-others my-next-others - 
                                                                                  others-moving 
                                                                              set susceptibles-moving 
                                                                                  susceptibles-moving + my-next-others / 
                                                                                  2 
                                                                              ...
                                                                              ask just-created-agents 
                                                                                  [ set xcor xcor 
                                                                                    ...
                                                                                    follow-path but-first path path 
                                                                                                repeat-count ] ] 
                                                                    [ 
                                                                      show ( list "Scheduled impossible movement" 
                                                                             "number-moving" number-moving "total-here" 
                                                                             total from-population from-place shape time ) ] ] ] ] ] 
                             [ 
                               show ( list "no population at scheduled movement" from-place time shape ? ) ] ] ] 
end

to SET-UP-INDEX-CASE-02953 
   let index-case-data [ ] 
   if-else index-case = "Funston" 
           [ set index-case-data the-index-cases-camp-funston ] 
           [ set index-case-data the-index-cases-etaples ] 
   forEach index-case-data 
           [ let day item 0 ? 
             let year item 1 ? 
             let longitude item 2 ? 
             let latitude item 3 ? 
             ...
             if not is-agent? population 
                [ set population closest-land-population x y number-infected ] 
             if-else is-agent? population 
                     [ ask population 
                           [ set my-infected number-infected ] 
                       if uninitialised? the-first-day 
                          [ set the-first-day day ] 
                       if uninitialised? the-first-year 
...
to SCHEDULE-ODDS-CHANGES-02955 
...
to ARMISTICE-CELEBRATIONS-NORTH-AMERICAN-AND-EUROPE-02957 
   if ( is-historical? ) 
      [ set the-encounter-rate-changes sentence the-encounter-rate-changes [ [ 314 1918 10 -166 46 27 76 ] [ 315 1918 
                                                                                                             2.5 -166 46 
                                                                                                             27 76 ] 
                                                                             [ 316 1918 2 -166 46 27 76 ] [ 317 1918 2 
                                                                                                            -166 46 27 
                                                                                                            76 ] ] ] 
end

to ARMISTICE-CELEBRATIONS-AUSTRALIA-AND-NEW-ZEALAND-02958 
   if ( is-historical? ) 
      [ set the-encounter-rate-changes sentence the-encounter-rate-changes [ [ 314 1918 10 -9 142 -48 -174 ] [ 315 1918 
                                                                                                               2.5 -9 
                                                                                                               142 -48 
                                                                                                               -174 ] 
                                                                             [ 316 1918 2 -9 142 -48 -174 ] [ 317 1918 2 
                                                                                                              -9 142 -48 
                                                                                                              -174 ] ] ] 
end

to SCHEDULE-ENCOUNTER-RATE-CHANGES-02959 
   forEach the-encounter-rate-changes 
           [ let day item 0 ? 
             ...
             let new-encounter-rate factor * the-encounter-rate 
             let populations objects with [ kind = "Population" ] 
             let time-of-change compute-time day year 
             do-at-time time-of-change 
                        task [ ask populations with [ xcor >= min-x and xcor <= max-x and ycor >= min-y and ycor <= 
                                                      max-y ] 
                                   [ set my-encounter-rate new-encounter-rate ] ] 
             do-at-time ( time-of-change + 1 ) 
                        task [ ask populations 
                                   [ set my-encounter-rate the-encounter-rate ] ] ] 
end

to COMPUTE-GLOBAL-INFECTED-02971 
   do-every ( 1 ) 
            task [ set the-global-infected maximum sum [ 
                                                         round my-infected ] of objects 0 ] 
end

to COMPUTE-GLOBAL-SUSCEPTIBLES-02972 
   do-every ( 1 ) 
            task [ set the-global-susceptibles maximum sum [ 
                                                             round my-susceptibles ] of objects 0 ] 
end

to COMPUTE-GLOBAL-RECOVERED-02973 
   do-every ( 1 ) 
            task [ set the-global-recovered maximum sum [ 
                                                          round my-recovered ] of objects 0 ] 
end

to VIEW-SUB-POPULATION-03004 
   do-every ( 1 ) 
            task [ if view-who != the-previous-view-who 
                      [ ask all-of-kind "Population" 
                            [ set color compute-color ] 
                        set the-previous-view-who view-who ] ] 
end

to MULTIPLE-PLOT-GENERATIONS-03010 
   set maximum-plot-generations 10000 
end

to PLOT-SUSCEPTIBLES-03011 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Populations" 
                                     create-temporary-plot-pen "Susceptible" 
                                     set-plot-pen-color darken-by-generation blue 
                                     set-plot-pen-mode 0 add-to-plot time log ( .1 + the-global-susceptibles ) 10 ] ] 
end

to PLOT-INFECTED-03012 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Populations" 
                                     create-temporary-plot-pen "Infected" 
                                     set-plot-pen-color darken-by-generation red 
                                     set-plot-pen-mode 0 add-to-plot time log ( .1 + the-global-infected ) 10 ] ] 
end

to PLOT-RECOVERED-03013 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Populations" 
                                     create-temporary-plot-pen "Recovered" 
                                     set-plot-pen-color darken-by-generation green 
                                     set-plot-pen-mode 0 add-to-plot time log ( .1 + the-global-recovered ) 10 ] ] 
end

to PLOT-DEATHS-03014 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Populations" 
                                     create-temporary-plot-pen "Deaths" 
                                     set-plot-pen-color darken-by-generation gray 
                                     set-plot-pen-mode 0 add-to-plot time log ( .1 + the-global-dead ) 10 ] ] 
end

to PLOT-DAILY-DEATHS-03016 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Daily Population Change" 
                                     create-temporary-plot-pen "Deaths" 
                                     set-plot-pen-color darken-by-generation gray 
                                     set-plot-pen-mode 0 add-to-plot time log ( maximum .1 daily-deaths ) 10 ] ] 
end

to PLOT-DAILY-INFECTED-03017 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Daily Population Change" 
                                     create-temporary-plot-pen "Infected" 
                                     set-plot-pen-color darken-by-generation red 
                                     set-plot-pen-mode 0 add-to-plot time log ( maximum .1 daily-infected ) 10 ] ] 
end

to PLOT-DAILY-RECOVERED-03018 
   do-at-time ( .000001 ) 
              task [ do-every ( 1 ) 
                              task [ set-current-plot "Daily Population Change" 
                                     create-temporary-plot-pen "Recovered" 
                                     set-plot-pen-color darken-by-generation green 
                                     set-plot-pen-mode 0 add-to-plot time log ( maximum .1 daily-recovered ) 10 ] ] 
end

to SUMMARIZE-OUTCOME-03019 
   when task [ the-global-infected < 1 and time > 1 ] 
        task [ output-print ( word "Total number of deaths due to the pandemic were " 
                              round the-global-dead " where history = '" history "'and index case = '" index-case "'." ) ] 
end

to PAUSE-WHEN-NO-ONE-INFECTED-03020 
   when task [ the-global-infected < 1 and time > 1 ] 
        task [ set stop-running true ] 
end

to PAUSE-WHEN-1-JAN-1920-03021 
...
to INTO-CAMP-03026 
   do-at-time ( 1 ) 
              task [ do-every ( 1 ) 
                              task [ to-camp my-nationality my-daily-flow ] ] 
end

to CLOSE-CAMP-03027 
   if ( is-historical? ) 
      [ when task [ the-first-year > 0 ] 
             task [ do-at-time ( ( compute-time 314 1918 ) + ( random-integer-between 7 30 ) ) 
                               task [ set my-daily-flow 0 
                                      set my-encounter-rate the-encounter-rate 
                                      if my-nationality = "Britain and Northern France" 
                                         [ set my-nationality "Britain" ] 
                                      from-camp my-nationality total-population * .99 ] ] ] 
end

to ENCOUNTER-RATE-03028 
   set my-encounter-rate 3 * the-encounter-rate 
end
