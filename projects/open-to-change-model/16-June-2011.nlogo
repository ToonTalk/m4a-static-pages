...
to setup
  ca
  crt number
  ask turtles [ set greed random 9 set wealth 100]
  ask patches [ set energy-on-patch random 9 set pcolor 60 + energy-on-patch ]
end
...
to go
  ...
  consume
  price-land
  ask turtles [ set wealth wealth - cost-of-living ]
  ask patches [ set price price - decay ]
  tick
end
...
to move
  ;; agents find cheapest neighboring patch and move to it if they have enough wealth
    ask turtles [
...
to consume
  ;; agents take energy according to their greed, only limited by the energy available on the patch
  ask turtles [
    ...
    [
      ...
      ask patch-here [ set energy-on-patch energy-on-patch - g ]
      ]
